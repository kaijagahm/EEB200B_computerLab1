...
driftFunction <- function(t = 5, L = 3, p = 0.1, N = 100){
  ...
  # Definenumber of chromosomes, since N is number of diploid individuals
  nalleles <- 2*N
...
```{r}
cat(paste0(sum(tail(driftResults, 1) == 0), " of the 1000 SNPs are at frequency 0 at the end of the simulation (in generation 10000)"))
```

b)  How many are at frequency 1?

```{r}
cat(paste0(sum(tail(driftResults, 1) == 1), " of the 1000 SNPs are at frequency 1 at the end of the simulation (in generation 10000)"))
```
...
Repeat the simulation, but this time set `p`=0.6. e) How many of the 1000 SNP are at frequency 0 at the end of the simulation (in generation 10000)?

```{r}
driftResults2 <- driftFunction(N = 100, L = 1000, t = 10000, p = 0.6)
cat(paste0(sum(tail(driftResults2, 1) == 0), " of the 1000 SNPs are at frequency 0 at the end of the simulation (in generation 10000)"))
```

e)  How many are at frequency 1?
```{r}
cat(paste0(sum(tail(driftResults2, 1) == 1), " of the 1000 SNPs are at frequency 1 at the end of the simulation (in generation 10000)"))
```
